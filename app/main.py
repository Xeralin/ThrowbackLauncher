...
def main() -> int:
    ...
    for folder in local_downloads():
        with contextlib.suppress(OSError):
            ...
            if (
                folder.name.endswith(HM_FOLDER_SUFFIX)
                and (folder / HELIOS_JSON).exists()
                and not (folder / HM_LAUNCHER).exists()
            ):
                shutil.copy2(HELIOS_DIR / HM_LAUNCHER, folder / HM_LAUNCHER)
    cfg = load_config()
