...
def installed_variant(key: str) -> tuple[Path, bool] | None:
    for is_hm in (False, True):
        path = DOWNLOADS_DIR / (hm_folder_name(key) if is_hm else key)
        if _is_installed(path):
            return path, is_hm
    return None


def partial_variant(key: str) -> bool | None:
    if installed_variant(key) is not None:
        return None
    for is_hm in (False, True):
        if (DOWNLOADS_DIR / (hm_folder_name(key) if is_hm else key)).is_dir():
            return is_hm
    return None
