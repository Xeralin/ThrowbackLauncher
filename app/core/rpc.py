...
class Presence:
    ...
    def drain(self) -> None:
        if not self.sock or _IS_WINDOWS:
            return
        try:
...
def _find_game_folder() -> str | None:
    ...
    for pid in running_game_pids():
        ...
        if cwd is None:
            continue
        try:
            rel = cwd.resolve().relative_to(downloads_root)
        except ValueError:
            continue
        if rel.parts:
            return rel.parts[0]
...
def _build_buttons(cfg: dict) -> list:
    ...
    if not buttons:
        buttons.append(
            {"label": "Rainbow Six on Linux", "url": "https://github.com/Xeralin/Throwback/"}
        )
    return buttons
