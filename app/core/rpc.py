...
def _apply_tokens(text: str, label: str, code: str, operation: str) -> str:
    return (
        text.replace("[season]", label)
        .replace("[code]", code)
...
def _build_activity(label: str, start_time: int, cfg: dict) -> dict:
    ...
    details = _apply_tokens(
        get_setting(cfg, "discord_details", "").strip(), label, code, operation
    )
    state_raw = get_setting(cfg, "discord_state", "").strip()
    state = _apply_tokens(state_raw, label, code, operation)
    large_image = get_setting(cfg, "discord_large_image", "").strip()
    large_text_raw = get_setting(cfg, "discord_large_text", "").strip()
    large_text = _apply_tokens(large_text_raw, label, code, operation) if large_text_raw else details
    small_image = get_setting(cfg, "discord_small_image", "").strip()
    small_text_raw = get_setting(cfg, "discord_small_text", "").strip()
    small_text = _apply_tokens(small_text_raw, label, code, operation) if small_text_raw else ""
    ...
    activity: dict = {"timestamps": {"start": start_time}}
    if details:
