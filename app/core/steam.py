...
from core.style import step_fail, step_warn
...
def is_steam_running() -> bool:
    for proc in psutil.process_iter(["name"]):
        if (proc.info["name"] or "").lower() in _STEAM_NAMES:
            return True
    return False
...
def running_game_pids() -> list[int]:
    ...
    for proc in psutil.process_iter():
        try:
            ...
            continue
        haystack = " ".join(cmdline) if cmdline else name
        if not _GAME_PROC_RE.search(haystack):
            continue
        if _IS_WINDOWS or proc_environ(proc.pid).get("STEAM_COMPAT_DATA_PATH"):
...
def _remove_grid_artwork(appid: int) -> None:
    ...
    for f in grid.iterdir():
        if f.name == f"{prefix}.json" or f.name.startswith(f"{prefix}_"):
            try:
                f.unlink()
            except OSError:
                pass
