...
def lookup_appid(appids: dict[str, int], exe: Path) -> int | None:
    appid = appids.get(str(exe))
    if appid is None and exe.name == LAUNCHER_EXE:
        appid = appids.get(str(exe.with_name(LEGACY_LAUNCHER_EXE)))
    return appid


def find_existing_appid(exe: Path) -> int | None:
    return lookup_appid(shortcut_appids(), exe)
...
def migrate_shortcut_exe(old_exe: Path, new_exe: Path) -> None:
    path = _shortcuts_path()
    if path is None or not path.exists():
        return
    try:
    ...
    changed = False
    for entry in parsed.get("shortcuts", {}).values():
        if entry.get("exe", "").strip('"') == str(old_exe):
            entry["exe"] = f'"{new_exe}"'
            changed = True
    if changed:
        _backup_and_write(path, _serialize_vdf(parsed))
