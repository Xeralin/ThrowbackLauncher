...
def ensure_hm_mod(hm_version: str, update: Callable[[str], None],
                  on_progress: Callable[[float], None] | None = None) -> Path | None:
    if hm_version == "latest":
    ...
    try:
        ...
        if rc.returncode != 0:
            ...
            return None
        archive_path.unlink()
        if not _hm_mod_complete(tmp_dir):
            _set_error(f"Heated Metal {tag} archive is incomplete")
            return None
        if mod_dir.exists():
...
def _apply_hm_mod(target_dir: Path, mod_dir: Path) -> None:
    ...
    hm_version_file(target_dir).write_text(mod_dir.name)
...
def apply_hm(target_dir: Path, username: str, hm_version: str | None,
    ...
    with (reporter or LazySpinner()) as sp:
        ...
        if hm_version is not None:
            mod_dir = ensure_hm_mod(hm_version, sp.update, on_progress=sp.progress)
            if mod_dir is None:
        ...
        sp.succeed("Heated Metal applied" if mod_dir is not None else "HeliosLoader applied")
        return True
