...
class UninstallController(QObject):
    ...
    _finished_in = Signal(bool, str)
    _item_finished_in = Signal(str, bool, str)

    def __init__(self, downloader: QObject, parent: QObject | None = None) -> None:
        super().__init__(parent)
        self._downloader = downloader
        self._busy_key: str | None = None
        self._finished_in.connect(self.finished)
        self._item_finished_in.connect(self.item_finished)
    ...
    def _dispatch(self, key: str) -> None:
        ...
        if self._blocked(key):
            self.finished.emit(False, BLOCKED_MESSAGE)
            return
        self._busy_key = key
        ...
        def work() -> None:
            ...
            self._busy_key = None
            self._finished_in.emit(bool(result.get("ok")), str(result.get("message", "")))
    ...
    def _dispatch_item(self, key: str, item: str) -> None:
        if self._busy_key is not None:
            return
        if self._blocked(key):
            self.item_finished.emit(item, False, BLOCKED_MESSAGE)
            return
        self._busy_key = key
        ...
        def work() -> None:
            ...
            self._busy_key = None
            self._item_finished_in.emit(item, bool(result.get("ok")), str(result.get("message", "")))
