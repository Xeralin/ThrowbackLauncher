...
class DownloadController(QObject):
    ...
    def _delete_partial(self, season_key: str) -> None:
        ...
        targets = [
            ...
            if (path := DOWNLOADS_DIR / folder).is_dir() and path != installed_path
        ]
        if not targets:
    ...
    def _run_next(self) -> None:
        if self._index >= len(self._commands):
            if not self._revalidated:
                ...
                self._index = 0
                self._set_progress(0.0)
                self.log_line.emit(f"Validating {self._target}")
                self._run_next()
                return
    ...
    def _steam_setup_begin(self, season_key: str, proton_index: int) -> None:
        ...
        threading.Thread(
            ...
            args=(self._generation, name, launcher, target, proton, icon, logo, hero, capsule),
            daemon=True,
        ).start()
