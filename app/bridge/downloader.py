...
from core.steam import (
    apply_steam_setup,
    is_game_running,
    is_steam_running as steam_running,
    list_protons,
...
class DownloadController(QObject):
    ...
    def set_peers(self, updater: QObject, settings: QObject, uninstaller: QObject) -> None:
        self._peer = updater
        self._settings = settings
        self._uninstaller = uninstaller
        updater.changed.connect(self._start_next)
        settings.steam_account_changed.connect(self._start_next)
    ...
    @Slot("QVariantList")
    def reorder_queue(self, keys: list) -> None:
        QTimer.singleShot(0, lambda: self._reorder_queue(list(keys)))

    def _reorder_queue(self, keys: list) -> None:
        by_key = {item[0]: item for item in self._queue}
        reordered = [by_key.pop(key) for key in keys if key in by_key]
    ...
    def _enter_paused_from_queue(self) -> bool:
        if not self._queue:
            return False
        key, hm, library = self._queue.pop(0)
        self._active_key = key
        self._enable_hm = hm
        partial = partial_variant(key)
        if partial is not None:
            self._target = partial[0]
        else:
            root = Path(library).resolve() if library else default_library()
            self._target = root / (hm_folder_name(key) if hm else key)
    ...
    def _set_paused(self, value: bool) -> None:
        ...
        if value:
            self._paused = True
            set_setting(self._cfg, "queue_paused", True)
    ...
    def _persist_queue(self) -> None:
        ...
        if (self._running or self._state == "paused") and not self._verifying and self._active_key:
            entries.insert(0, (self._active_key, self._enable_hm, str(self._target.parent)))
        set_setting(self._cfg, "queue", [
            {"key": key, "hm": hm, "library": library} for key, hm, library in entries
        ])
        save_config(self._cfg)
    ...
    def _start_next(self) -> None:
        if self._paused:
            return
        account = get_setting(self._cfg, "steam_account", "")
    ...
    def _delete_partial(self, season_key: str) -> None:
        if self._running and season_key == self._active_key:
            self.partial_deleted.emit(season_key, False, "Download is running — pause it first")
            return
    ...
    def _on_deleted(self, season_key: str, ok: bool, message: str) -> None:
        self._deleting_key = None
        if ok and season_key == self._active_key and not self._running:
    ...
    def _clear_paused_flag(self) -> None:
        if self._paused:
            self._paused = False
            set_setting(self._cfg, "queue_paused", False)
            save_config(self._cfg)

    def _launch(self, download: dict, season_key: str, target: Path,
                enable_hm: bool, verify: bool) -> None:
        if self._state == "paused" and self._active_key and self._active_key != season_key:
            self._queue.insert(0, (self._active_key, self._enable_hm, str(self._target.parent)))
            self.queue_changed.emit()
        self._clear_paused_flag()
        self._generation += 1
    ...
    def _steam_setup(self, generation: int, name: str, launcher: Path, target: Path, proton: dict,
        ...
        try:
            error = apply_steam_setup(name, launcher, target, proton, icon=icon, logo=logo, hero=hero,
                                      capsule=capsule)
        except (OSError, ValueError) as e:
            error = f"Could not update Steam files — {e}"
        if error is None:
            self._steam_done.emit(generation, True, f"{name} was added to Steam")
        else:
            self._steam_done.emit(generation, False, error)
    ...
    def _finish(self, code: int) -> None:
        ...
        if self._cancelled:
            outcome = ""
            if self._paused and not self._verifying and self._active_key:
                self._set_state("paused")
            else:
                self._clear_paused_flag()
                self._set_state("idle")
