...
class Shears(QObject):
    ...
    def _cut(self, key: str, kind: str, level: int) -> None:
        if kind not in _KINDS:
            self._cut_in.emit({"key": key, "ok": False, "message": "Invalid target", "freed": 0, "scan": _empty()})
            return
        variant = installed_variant(key)
        if variant is None or variant[1]:
            self._cut_in.emit({"key": key, "ok": False, "message": "Not available", "freed": 0, "scan": _empty()})
            return
        if is_game_running():
            self._cut_in.emit({
                "key": key,
