...
class Shears(QObject):
    ...
    def _scan(self, key: str) -> None:
        ...
        if variant is None or variant[1]:
            self._scan_in.emit(_empty())
            return
        self._scan_in.emit(_serialize(scan_download(variant[0])))
    ...
    def _cut(self, key: str, kind: str, level: int) -> None:
        if kind not in _KINDS:
            self._cut_in.emit({"ok": False, "message": "Invalid target", "freed": 0, "scan": _empty()})
            return
        variant = installed_variant(key)
        if variant is None or variant[1]:
            self._cut_in.emit({"ok": False, "message": "Not available", "freed": 0, "scan": _empty()})
            return
        if is_game_running():
            self._cut_in.emit({
                "ok": False,
        ...
        self._cut_in.emit({"ok": True, "message": "", "freed": freed, "scan": _serialize(scan_download(path))})
