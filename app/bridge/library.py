...
from core.steam import lookup_appid, shortcut_appids
...
class Library(QObject):
    ...
    def home(self) -> list:
        ...
        for folder, installed in sorted(folders, key=lambda entry: not entry[1]):
            self._merge(entries, folder, partial=not installed, appids=appids)
    ...
    def _merge(self, entries: dict, folder: Path, partial: bool, appids: dict) -> None:
        ...
        if is_hm:
            entry["hm"] = True
        if not partial and lookup_appid(appids, folder / launcher_name(is_hm)) is not None:
