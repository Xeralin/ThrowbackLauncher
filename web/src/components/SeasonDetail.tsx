...
import { SEASON_INFO } from "@/config/season-info";
          ...
          <button
            ...
            className={`flex items-center justify-between gap-3 rounded-md border bg-surface-2 px-3 py-2 text-left transition ${
              library.path === selected
                ? "border-border-brand shadow-[0_2px_18px_var(--color-brand-glow)]"
                : "border-border"
            } ${
              library.exists
...
export function SeasonDetail({
  ...
  const [dlPrompt, setDlPrompt] = useState<false | "choice" | "library">(false);
  ...
  function openDownloadPrompt() {
    ...
    setDlHm(false);
    setDlPrompt(season.heatedMetal ? "choice" : "library");
  }

  function chooseVariant(hm: boolean) {
    if (multiLib) {
      setDlHm(hm);
      setDlPrompt("library");
      return;
    }
    startDownload(hm, dlLibrary);
  }
  ...
  const statusRow = (
    <div className="mt-4 flex items-center gap-4">
      ...
      {statusLine && (
        <p className="min-w-0 truncate rounded-[4px] border border-border bg-surface-2 px-[0.4em] py-[0.1em] font-mono text-label text-text">
          {statusLine}
        </p>
      )}
    </div>
  ...
  const tabs: TabItem<TabId>[] = [
    ...
    ...(info ? [{ id: "info" as const, label: "Info" }] : []),
  ];
    ...
    <>
      <div
        className={
          tab === "info"
            ? "flex flex-col"
            : "flex h-[calc(100dvh_-_var(--topbar-h)_-_3rem)] flex-col min-[100em]:h-[calc(100dvh_-_var(--topbar-h)_-_5rem)]"
        }
      >
        <BackHeading title={`${season.code} ${season.name}`} onBack={onBack} />

        <div className="relative mb-3 h-[214px] shrink-0 overflow-hidden rounded-lg border border-border">
          {season.splash ? (
            <Image
              src={season.splash}
              alt=""
        ...
        {running && tab !== "info" && statusRow}

        {tab === "info" && info ? (
          <div className="mt-2">
            <SeasonInfo entry={info} />
          </div>
        ) : (
          <div className="mt-3 flex min-h-24 flex-1 flex-col">
            <LogBox lines={deferredLog} className="flex-1" />
          </div>
        )}
      </div>
