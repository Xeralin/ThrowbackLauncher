...
import { onBridgeEvent, useSettings, useUpdate } from "@/lib/bridge";
...
function UpdateRedirect() {
  ...
  useEffect(() => {
    if (done.current || update.components.length === 0) return;
    done.current = true;
    if (normalizePath(pathRef.current) === "/") router.push("/updates");
  }, [update.components.length, router]);
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      ...
      if (parent && normalizePath(parent) !== current) router.push(parent);
    }
