...
type TransferState = "downloading" | "queued" | null;
    ...
    <div
      ...
      className={`group relative h-full rounded-lg border border-border bg-surface transition-[border-color,box-shadow,opacity] duration-200 ${
        editing
          ...
          : "cursor-pointer overflow-hidden hover:border-border-brand card-glow-hover card-line-hover"
      ...
      >
        <div className="min-w-0">
          ...
          <div className="mt-1.5">
            <SeasonTags season={season} transfer={transfer} />
          </div>
        </div>
        {action &&
          ("steam" in action ? (
...
export function SeasonBrowser({
  ...
  const open = useCallback((season: Season) => {
    setReturnScroll(window.scrollY);
    setDirection("forward");
    setSelected(season);
    window.history.pushState({ tbSeason: season.key }, "");
  }, []);
    ...
    setDetail(
      selected
        ? {
            ...
            seasonKey: selected.key,
            reset: back,
          }
        : null,
    );
    return () => setDetail(null);
  }, [selected, setDetail, back]);
  ...
  useEffect(() => {
    const onPop = (event: PopStateEvent) => {
      const key = (event.state as { tbSeason?: string } | null)?.tbSeason;
      const target = key ? (seasons ?? []).find((s) => s.key === key) : null;
      if (target) {
        setDirection("forward");
        setSelected(target);
      } else if (selected) {
  ...
  useEffect(() => {
    ...
    function openByKey(key: string) {
      const target = seasons?.find((season) => season.key === key);
      if (target && target.key !== selected?.key) open(target);
    }
    const pending = window.sessionStorage.getItem("tb-open-season");
    if (pending && seasons.some((season) => season.key === pending)) {
      window.sessionStorage.removeItem("tb-open-season");
      openByKey(pending);
    }
    function onOpen(event: Event) {
      openByKey((event as CustomEvent).detail as string);
    }
    window.addEventListener("throwback:open-season", onOpen);
    return () => window.removeEventListener("throwback:open-season", onOpen);
  }, [seasons, selected, open]);

  function transferState(season: Season): TransferState {
    if (dl.running && dl.activeKey === season.key) return "downloading";
    if (dl.queue.includes(season.key)) return "queued";
    return null;
  }

  function cardAction(season: Season): CardAction | null {
    if (season.partial) {
      if (transferState(season) === "downloading") {
        return dl.state === "downloading"
          ? { label: "Pause", run: () => dl.cancel(), primary: false }
          : null;
      }
      if (transferState(season) === "queued") {
        return {
          label: "Queued",
          run: () => dl.dequeue(season.key),
          primary: false,
        };
      }
      return {
        label: "Continue",
        run: () => dl.enqueue(season.key, season.hm, ""),
        primary: false,
      };
    }
    if (dl.running && dl.activeKey === season.key) {
      return dl.state === "downloading"
        ? { label: "Cancel", run: () => dl.cancel(), primary: false }
        : null;
    }
  ...
  if (seasons === null || visible === null) {
    ...
    listContent = (
      <div
        className="grid auto-rows-[196px] grid-cols-2 gap-4 max-[40em]:grid-cols-1 min-[80em]:grid-cols-3"
    ...
    listContent = (
      <div className="-mx-8 -mb-6 -mt-6 flex flex-col max-[48em]:-mx-5 max-[48em]:-mb-5 max-[48em]:-mt-5 min-[100em]:-mx-12 min-[100em]:-mb-10 min-[100em]:-mt-10">
        {visible.map((season) => (
          <BannerCard
            key={season.key}
            season={season}
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`transition-colors ${
                    editing ? "text-brand" : "text-text-muted hover:text-text"
                  }`}
                >
                  <svg
                    viewBox="0 0 24 24"
