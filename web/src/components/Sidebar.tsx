...
function DownloadStatus({
  ...
  return (
    <button
      onClick={() => {
        onNavigate?.();
        if (detail?.seasonKey === season.key) return;
        if (pathname === "/download") {
          window.dispatchEvent(
            new CustomEvent("throwback:open-season", { detail: season.key }),
          );
        } else {
          window.sessionStorage.setItem("tb-open-season", season.key);
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-between gap-2 px-2.5 pt-[2px]">
        ...
        <span className="shrink-0 font-mono text-[0.65rem] leading-none text-white">
          ...
          {dl.queue.length > 0 && (
            <span className="text-text-muted"> +{dl.queue.length}</span>
          )}
        </span>
      </div>
    </button>
