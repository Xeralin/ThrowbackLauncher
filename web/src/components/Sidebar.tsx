...
function PauseIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
      <rect x="6.75" y="5" width="3.5" height="14" rx="1.75" />
      <rect x="13.75" y="5" width="3.5" height="14" rx="1.75" />
    </svg>
  );
}
...
function DownloadCard({
  ...
  return (
    <div
      ...
      className="relative mx-2 mb-2 h-12 w-[calc(100%-1rem)] cursor-pointer overflow-hidden rounded-md border border-border text-left"
    >
      {season.splash ? (
        <Image
          src={season.splash}
      ...
      <div className="absolute inset-0 flex items-center justify-between gap-2 px-2.5 pt-[2px]">
        ...
        <span className="flex shrink-0 items-center gap-2">
          ...
          {onToggle && (
            <button
              aria-label={paused ? "Resume" : verifying ? "Cancel" : "Pause"}
              onClick={(event) => {
