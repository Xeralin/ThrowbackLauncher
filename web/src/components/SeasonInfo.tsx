...
function InfoBox({
  ...
  return (
    ...
    >
      <p className="border-b border-border bg-surface-2 px-[0.6rem] py-[0.2rem] font-mono text-[0.66rem] uppercase tracking-[0.05em] text-text-muted">
        {title}
      </p>
      {children}
    </div>
...
export function SeasonInfo({ entry }: { entry: SeasonInfoEntry }) {
  ...
  const noteContent = entry.note ? (
    <Note>
      ...
      {entry.noteLink && (
        <>
          {" "}
          <Link href={entry.noteLink.href}>{entry.noteLink.text}</Link>
        </>
      )}
    </Note>
  ) : null;
  ...
  const note =
    noteContent && entry.notePlatform === "windows" ? (
      <OnWindows>{noteContent}</OnWindows>
    ) : noteContent && entry.notePlatform === "linux" ? (
      <OnLinux>{noteContent}</OnLinux>
    ) : (
      noteContent
    );
  ...
  const released = (
    <InfoBox title="Released" className="mt-[0.37rem]">
      <p className="px-[0.6rem] py-[0.2rem] text-[0.78rem] leading-[1.45] text-text-muted">
        {entry.release}
      </p>
    </InfoBox>
  );
