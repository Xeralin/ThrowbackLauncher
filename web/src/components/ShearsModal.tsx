...
export function ShearsModal({
  ...
  const actions: { key: string; label: string; freed: number; run: () => void }[] =
    [];
  if (videos > 0) {
    actions.push({
      key: "videos",
  ...
  return (
    ...
    >
      <p className="-mt-2 mb-4 text-ui text-text-muted">
        {season.name}
        {scan && <> · {formatBytes(scan.total)} on disk</>}
      </p>
      ...
      {notice && (
        <p className="mt-3 font-mono text-label text-text-muted">{notice}</p>
      )}
    </Dialog>
  );
}
