...
fn mirror_regions_for_build(build: &str) -> Option<&'static [MirrorRegion]> {
    let prefix = match build.find('_') {
        Some(i) => &build[..i],
        None => build,
    };
    for ms in MIRROR_SEASONS {
...
impl Engine {
    ...
    fn inject_library_w(&self, path: &str) -> bool {
    ...
    fn mirror_dll_path(&self) -> Option<String> {
        if let Ok(env) = std::env::var("RUNNER_MIRROR_DLL") {
            if !env.is_empty() {
                return Some(env);
            }
        }
        ...
        let dir = exe.parent()?;
        Some(dir.join("Mirror.dll").to_string_lossy().into_owned())
    }

    fn mirror_arm_pages(&self, regions: &[MirrorRegion]) -> i32 {
        let mut armed = 0;
    ...
    fn do_mirror(&self, build: &str, dry: bool, arm: bool) -> i32 {
        ...
        let text_rva = (text_base - self.base) as u32;
        let mod_hi = self.base + self.modsize as u64;
        let regions = mirror_regions_for_build(build);
        let nreg = regions.map(|r| r.len()).unwrap_or(0);

        let mut buf = vec![0u8; text_size as usize];
        let mut off = 0u32;
        while off < text_size {
            let want = (text_size - off).min(0x100000);
            let got =
                self.read_mem(text_base + off as u64, &mut buf[off as usize..(off + want) as usize]);
            if got == 0 {
                break;
            }
            off += got as u32;
        }
        let maxfix = (text_size / 4 + 16) as usize;
        let fix = crate::scanrip::scan_rip_rel(&buf, text_base, self.base, mod_hi, maxfix);
        let nfix = fix.len();

        if dry {
            println!("{{\"mode\":\"mirror-scan\",\"build\":\"{}\",\"textBase\":\"0x{:x}\",\"textSize\":{},\"read\":{},\"fixups\":{},\"regions\":{}}}",
                build, text_base, text_size, off, nfix, nreg);
            return 0;
        }
        if nreg == 0 {
            println!(
                "{{\"error\":\"no mirror regions for build\",\"build\":\"{}\"}}",
                build
            );
            return 1;
        }
        ...
        for r in regions {
            if r.offset >= text_rva as u64 {
                let o = (r.offset - text_rva as u64) as usize;
                if o + r.bytes.len() <= text_size as usize {
                    buf[o..o + r.bytes.len()].copy_from_slice(r.bytes);
                }
            }
        }
        let copy_base = self.allocate_near(text_base, text_size);
        let diff = copy_base as i64 - text_base as i64;
        if copy_base == 0 || diff > 0x3FFFFFFF || diff < -0x3FFFFFFF {
            println!(
                "{{\"error\":\"allocate_near out of range\",\"copyBase\":\"0x{:x}\"}}",
                copy_base
            );
            return 1;
        }
        for &fi in &fix {
            let fi = fi as usize;
            let v = crate::scanrip::sr_read_i32(&buf, fi);
            let nv = ((v as i64) - diff) as i32;
            let nvb = nv.to_le_bytes();
            buf[fi] = nvb[0];
            buf[fi + 1] = nvb[1];
            buf[fi + 2] = nvb[2];
            buf[fi + 3] = nvb[3];
        ...
        if wrote == 0 || wr != text_size as usize {
            println!("{{\"error\":\"write copy failed\"}}");
            return 1;
        }
        let mut payload = [0u8; 40];
        payload[4] = 1;
        payload[8..16].copy_from_slice(&text_base.to_le_bytes());
        payload[16..24].copy_from_slice(&copy_base.to_le_bytes());
        payload[24..32].copy_from_slice(&(text_size as u64).to_le_bytes());
        let name: Vec<u16> = "MirrorRegions"
            .encode_utf16()
        ...
        if !map.is_null() {
            ...
            if !view.Value.is_null() {
                unsafe { core::ptr::copy_nonoverlapping(payload.as_ptr(), view.Value as *mut u8, 40) };
                unsafe { UnmapViewOfFile(view) };
            }
        }
        let injected = match self.mirror_dll_path() {
            Some(dll) => self.inject_library_w(&dll),
            None => false,
        };
        let armed = if arm { self.mirror_arm_pages(regions) } else { 0 };
        if !map.is_null() {
            unsafe { CloseHandle(map) };
        }
        println!("{{\"mode\":\"mirror\",\"build\":\"{}\",\"textBase\":\"0x{:x}\",\"copyBase\":\"0x{:x}\",\"regions\":{},\"fixups\":{},\"injected\":{},\"armedPages\":{}}}",
            build, text_base, copy_base, nreg, nfix, if injected {"true"} else {"false"}, armed);
        if injected {
            0
        } else {
            1
...
impl Daemon {
    ...
    fn attach_tick(&mut self) {
        ...
        let mirror_enabled = mirror_regions_for_build(&self.build)
            .map(|r| !r.is_empty())
            .unwrap_or(false);
        if mirror_enabled && !self.mirror_injected {
            self.mirror_injected = self.eng.do_mirror(&self.build, false, false) == 0;
        }
        if self.pending {
            self.set_loading_status();
            let ready = if mirror_enabled {
                self.mirror_injected
            } else {
                self.is_ready()
            };
            if !ready {
        ...
        if !self.applied {
            ...
            if mirror_enabled {
                if let Some(r) = mirror_regions_for_build(&self.build) {
                    self.eng.mirror_arm_pages(r);
                }
            }
